...
echo $warning;
		...
		if(empty($imgFile)){
			...
			if($user->insertrepair($ran,$name,$branch,$dept,$acc,$desc,$userpic,$warning)){
				header('Location:https://repairit.tkn.co.th/line.php?key='.$ran.'&u='.$name.'&acc='.$acc.'&desc='.$desc.'');
			...
			if($stmt2->rowCount() > 0){
				...
				while($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)){
					...
					if($stmt->execute()){
						$successMSG = "NEW record succesfully inserted ...";
						echo $successMSG;
						header('Location:https://repairit.tkn.co.th/line.php?key='.$ran.'&u='.$name.'&acc='.$acc.'&stock=มีการเบิกอุปกรณ์');
					}else{
						$errMSG = "error while inserting ...";
						echo $errMSG;
						header('Location:'.$actual_link.'&st=fail');
